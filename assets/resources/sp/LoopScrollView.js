...
cc.Class({
    ...
    clearV(){
        ...
        this.nowFristDataIdx = 0; //头部数据下标
        this.nowFianlDataIdx = 0; //尾部数据下标
        this.dataFristItemNode = false;
        this.dataFianlItemNode = false;
        this.nowDataArr = [];
    },
    ...
    startBrakeMove() {
        ...
        this.brakeMoveTimeOut = setTimeout(function () {
            if (!instance || !instance.cantBrakeMove) {
                return;
            }
    ...
    sortBtnPosition(btnArr, anchorY, isTopBegin, callback) {
        ...
        btnArr.reduce((perValue, tNode, nodeIdx) => {
            ...
            tNode.y = nodeY;
            if (callback) {
                callback(tNode, nodeIdx + this.nowFristDataIdx, nodeIdx == nodeArrLen - 1, true); //节点 数据下标 是否初始创建时
            }
